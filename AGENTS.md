...
Etherna Authentication is a set of **.NET client libraries** (NuGet packages) to authenticate users against the Etherna SSO server — it is a library solution, not a runnable application. Three packages: `Etherna.Authentication` (common constants, abstractions, and the OpenID Connect client base), `Etherna.Authentication.AspNetCore` (code-flow authentication for ASP.NET Core applications), and `Etherna.Authentication.Native` (sign-in services and user token management for native .NET applications, with code and api-key/password flows).
...
Solution `EthernaAuthentication.sln`, three library projects. Project folders are named `EthernaAuthentication*`, root namespaces use `Etherna.Authentication[.<Module>]`; under the root, the namespace mirrors the folder path.
