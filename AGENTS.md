...
- **`src/EthernaAuthentication`** (`Etherna.Authentication`) — Core constants and abstractions shared by the other packages; offers no client registration by itself. Holds the Etherna constants (`EthernaDefaults`, `EthernaClaimTypes`, `EthernaScopes` — kept in sync with the SSO server), the OIDC client contract and base (`IEthernaOpenIdConnectClient`, `EthernaOpenIdConnectClientBase`, reading typed values from user claims via `Get…Async`/`TryGet…Async` pairs), `IDiscoveryDocumentService`/`DiscoveryDocumentService`, and `ClaimJsonSerializerContext` (source-generated JSON for AOT). A claim missing from a user principal is looked up on the userinfo endpoint, once per access token: the outcome is cached together with the token it was obtained with, and a different current token (a refresh, a new sign in) runs a new lookup, because the Native client is a singleton outliving tokens and users, while the scoped ASP.NET Core one sees one token per request. The cached outcome is a single immutable record, replaced in one step: keep it so, the singleton serves concurrent callers. An error there (an error response, a failed request, or no answer within the 15 seconds of `UserInfoTimeout`) is logged as a warning and cached as no claims, so the `TryGet…Async` methods keep answering null instead of throwing; a failure telling nothing about the token (a failed request, no answer, a 5xx) is cached only for the minute of `UserInfoFailureRetryDelay`, then the endpoint is asked again. `IsUserTokenRejectedAsync` shares the same lookup and tells the hosts when the endpoint answered 401, the token not being valid anymore. `UserInfoFailureRetryDelay` and `UserInfoTimeout` are internal, overridden only by the unit tests (`InternalsVisibleTo`). Log messages are `LoggerMessage` delegates in `Extensions/LoggerExtensions`. Depends on `IdentityModel` and `Microsoft.Extensions.Logging.Abstractions` only.
- **`src/EthernaAuthentication.AspNetCore`** (`Etherna.Authentication.AspNetCore`) — Code-flow client for ASP.NET Core applications. Registration via the `AddEthernaOpenIdConnect` overloads on `AuthenticationBuilder`, which also register automatic user access token management (`Duende.AccessTokenManagement.OpenIdConnect`) bound to the registered scheme, unless the application already registered its own; token refresh requires the application to request the `offline_access` scope. Its `EthernaOpenIdConnectClient` reads claims from the current `HttpContext`, consumes the managed user access token (falling back to the raw session token for plain bearer-authenticated requests), and is registered **scoped** because of its user claims cache.
...
- **`src/EthernaAuthentication.Native`** (`Etherna.Authentication.Native`) — Client for native .NET applications, with user token management built on `Duende.AccessTokenManagement.OpenIdConnect` (`LocalUserTokenStore`, `LocalUserAccessTokenRetriever`). Registration via the single `AddEthernaOidcClient` on `IServiceCollection`, which wires **both** sign-in flows together: `CodeFlow/` (`EthernaCodeSignInService` + `SystemBrowser`/`LoopbackHttpListener` — opens the system browser and receives the SSO redirect on a local listener) and `PasswordFlow/` (`EthernaApiKeySignInService` — api-key authentication without user interaction, on the SSO client dedicated to api keys). The flow is selected only at sign-in time through the `IEthernaSignInService` facade (`EthernaSignInService`), choosing the `SignInAsync()` / `SignInAsync(apiKey)` overload — no registration-time commitment (EAUTH-21). Each flow runs on its own OpenID Connect scheme (constants in `EthernaNativeDefaults`) because the client ids differ, so sign-in and token refresh always pass the scheme of the flow in use explicitly; the code flow scheme is also the default challenge scheme, required by Duende token management when more than one scheme is registered.
...
- **Package icon**: every packable `.csproj` declares `PackageIcon` over the root `icon.png`, packed as content beside the license texts and the readme. It is the Etherna symbol (the eye/node, without lettering: these packages carry the company brand, having no product mark of their own), cyan `#00ABBF` on a transparent 256 px square canvas, centred, with its width spanning 224 px — legible on the light and on the dark themes of nuget.org and of the IDE package managers. The file is derived from the vector symbol of the brand kit (`logo-symbol.svg`, https://info.etherna.io/brand-kit/), which is not square: a change to the artwork regenerates it from there, rendering the vector and never scaling a raster, and never by editing the derived file.
- **Readme images**: `README.md` is packed as the readme of every package, and nuget.org renders only images hosted on its allowed domains (`img.shields.io`, `raw.githubusercontent.com`...): relative paths and other hosts are dropped, with a warning shown to the package owners. Every image in the readme is an absolute URL on an allowed host; the LGPL logo is the copy in `doc/lgplv3-with-text-154x68.png`, referenced through `raw.githubusercontent.com` on `main`.
...
- Extension methods for registration: on `AuthenticationBuilder` (`AddEthernaOpenIdConnect`) and on `IServiceCollection` (`AddEthernaOidcClient`)
